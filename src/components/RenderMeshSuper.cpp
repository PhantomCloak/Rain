#include "RenderMeshSuper.h"
#include <vector>

RenderMeshSuper::RenderMeshSuper(WGPUDevice device, WGPURenderPipeline pipe, WGPUTexture texture, WGPUTextureView textureView, WGPUSampler sampler) {
...
RenderMeshSuper::~RenderMeshSuper() {
...
void RenderMeshSuper::updateBuffer(WGPUQueue queue) {
  wgpuQueueWriteBuffer(queue, uniformBuffer, 0, &uniform, sizeof(RenderMeshUniform));
}

void RenderMeshSuper::createUniformBuffer(WGPUDevice device) {
...
void RenderMeshSuper::SetVertexBuffer(WGPUDevice device, WGPUQueue queue, std::vector<VertexAttributes> vertexData) {
...
void RenderMeshSuper::createVertexBuffer(WGPUDevice device, int size) {
  vertexBufferDesc = {};
...
void RenderMeshSuper::createBindGroup(WGPUDevice device, WGPUBindGroupLayout bindGroupLayout, WGPUTextureView textureView, WGPUSampler sampler) {
...
void RenderMeshSuper::SetRenderPass(WGPURenderPassEncoder renderPass) {

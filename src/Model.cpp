...
Ref<MeshE> Model::processMesh(aiMesh* mesh, const aiScene* scene) {
  ...
  std::vector<VertexE> uniqueVertices;
  ...
  for (unsigned int i = 0; i < mesh->mNumFaces; i++) {
    ...
    for (unsigned int j = 0; j < face.mNumIndices; j++) {
      ...
      uniqueIndices.push_back(uniqueVertices.size() - 1);  // Point to the new vertex in the uniqueVertices vector
    }
  }

  vertices = std::move(uniqueVertices);
  indices = std::move(uniqueIndices);
